...
export class Logger {
   ...
   */
  private log(level: LogLevelValue, levelStr: string, color: string, message: string, ...args: any[]): void {
    ...
    console.log(`${this.colorize(timestamp, 'dim')} ${coloredLevel} ${coloredMessage}`, ...args);
  }
   ...
   * Log ReAct Action
   */
  action(tool: string, parameters: Record<string, any>): void {
    if (!this.shouldLog('INFO')) return;

    console.log(`${this.colorize('🔧 ACTION:', 'magenta')} ${this.colorize(tool, 'bright')}`);
    if (Object.keys(parameters).length > 0) {
      console.log(this.colorize('Parameters:', 'dim'), parameters);
    }
    console.log();
  }
   ...
   */
  observation(summary: string, details?: any): void {
    if (!this.shouldLog('INFO')) return;
